...
const Cart = () => {
  ...
  const totalKg = useMemo(() => {
    ...
    safeItems.forEach((item) => {
      const pack = (item.selectedPack || "").toLowerCase().trim();
  ...
  const discountPct = useMemo(() => getDiscount(totalKg), [getDiscount, totalKg]);
  const discountAmt = useMemo(() => (totalPrice * discountPct) / 100, [totalPrice, discountPct]);
  const finalPrice = useMemo(() => totalPrice - discountAmt, [totalPrice, discountAmt]);
  ...
  const buildOrderText = () => {
    ...
    safeItems.forEach((item) => {
      ...
      text += `• ${item.product.name}${
        item.selectedBrand ? ` (${item.selectedBrand})` : ""
      } (${item.selectedPack}) x${item.quantity} — ₹${(price * item.quantity).toFixed(0)}\n`;
    });
  ...
  const generatePDF = () => {
    ...
    const rows = safeItems
      .map((item) => {
        const price = getProductPrice(item.product, item.selectedBrand);
        return `<tr>
          <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px">${item.product.name}</td>
          <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px">${item.selectedBrand || "-"}</td>
          <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px">${item.selectedPack}</td>
          ...
          <td style="padding:10px 12px;border-bottom:1px solid #eee;font-size:13px;text-align:right;font-weight:600">₹${(price * item.quantity).toFixed(0)}</td>
        </tr>`;
      })
      .join("");
    ...
    <p style="font-size:13px;color:#666">Date: ${new Date().toLocaleDateString("en-IN", {
      day: "numeric",
      month: "long",
      year: "numeric",
    })}</p>
    ...
    <div class="summary">
      ...
      ${
        discountPct > 0
          ? `<div class="summary-row" style="color:#059669"><span>Bulk Discount (${discountPct}%)</span><span>-₹${discountAmt.toFixed(0)}</span></div>`
          : ""
      }
    ...
    return (
      <Layout>
        <div className="container max-w-7xl mx-auto px-4 sm:px-6 py-24 text-center">
          ...
          <Link
            to="/products"
            className="inline-flex items-center gap-2 rounded-2xl bg-primary px-7 py-3.5 font-bold text-primary-foreground shadow-lg shadow-primary/20 hover:scale-[1.03] transition-transform"
          >
            Browse Products
          </Link>
        </div>
        ...
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground mt-1">
              {safeItems.length} item{safeItems.length > 1 ? "s" : ""} · Est. {totalKg.toFixed(1)} kg
            </p>
          </div>
          <button
            onClick={clearCart}
            className="text-xs sm:text-sm text-destructive hover:underline font-medium flex items-center gap-1.5"
          >
        ...
        <div className="grid gap-6 lg:gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-2 sm:space-y-3 max-h-[60vh] lg:max-h-[70vh] overflow-y-auto pr-1">
            {safeItems.map((item) => {
              const price = getProductPrice(item.product, item.selectedBrand);
              const img =
                getProductImageUrl(item.product.imagePath) ||
                getCategoryImageUrl(item.product.category);

              const cartItemKey = `${item.product.id}-${item.selectedPack}-${item.selectedBrand ?? "no-brand"}`;
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="h-7 w-7 sm:h-8 sm:w-8 rounded-lg glass-card flex items-center justify-center hover:bg-muted transition-colors"
                    >
                      <Minus className="h-3 w-3" />
                    </button>

                    <span className="text-xs sm:text-sm font-bold w-6 sm:w-7 text-center tabular-nums">
                      {item.quantity}
                    </span>
                  ...
                  <p className="text-xs sm:text-sm font-bold text-primary w-12 sm:w-16 text-right tabular-nums">
                  ...
                  <button
                    ...
                    className="text-destructive/60 hover:text-destructive hover:bg-destructive/10 p-1.5 sm:p-2 rounded-lg transition-colors"
                  >
          ...
          <div className="space-y-4">
            <div className="rounded-2xl glass-card p-5 sm:p-6 sticky top-20">
                ...
                {discountPct > 0 && (
                  <div className="flex justify-between text-secondary">
                    <span className="flex items-center gap-1">
                      <CheckCircle2 className="h-3 w-3" /> Bulk ({discountPct}%)
                    </span>
              ...
              <div className="mt-4 rounded-xl bg-muted/50 p-3">
                <p className="text-[10px] sm:text-xs font-bold text-foreground mb-2">
                  Bulk Discounts
                </p>
                  ...
                  {[
                    ...
                    <div
                      ...
                      className={`flex items-center gap-2 ${
                        totalKg >= tier.kg ? "text-secondary font-semibold" : ""
                      }`}
                    >
                      {totalKg >= tier.kg ? (
            ...
            <div className="rounded-2xl glass-card p-4 sm:p-5">
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                >
                  ...
                  <span className="text-[8px] sm:text-[9px] font-bold text-secondary bg-secondary/15 px-2 py-0.5 rounded-full shrink-0">
                    Available
                  </span>
                </button>
                ...
                <div className="opacity-50 space-y-2">
                  {[
                    ...
                    >
                      ...
                      <span className="text-[8px] sm:text-[9px] font-semibold text-muted-foreground bg-muted px-2 py-0.5 rounded-full shrink-0">
                        Soon
                      </span>
                    </div>
                  ))}
        ...
        {codDialogOpen && (
          ...
          >
            <div
              ...
              onClick={() => !codLoading && setCodDialogOpen(false)}
            />
            <motion.div
              ...
              {codSuccess ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Your COD order has been confirmed. We'll contact you shortly.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <p className="text-xs text-muted-foreground mb-4">
                    Fill in delivery details to place your order
                  </p>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="rounded-xl bg-muted/40 p-3">
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        {safeItems.length} items · {totalKg.toFixed(1)} kg
                        {discountPct > 0 ? ` · ${discountPct}% bulk discount` : ""}
                      </p>
                    </div>
                        ...
                        try {
                          ...
                          const orderItems = safeItems.map((item) => {
                            ...
                            return {
                              ...
                              brand_name: item.selectedBrand || null,
                              pack_size: item.selectedPack,
            ...
            >
              ...
              {emailSent ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Your cart summary is ready to send.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <p className="text-xs text-muted-foreground mb-4">
                    Send your cart details to any email address
                  </p>
