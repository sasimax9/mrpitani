...
import {
  Tag, Truck, CalendarCheck, Headset, CheckCircle2, MessageCircle,
  Mail, X, Building2, User, Phone, MapPin, Package, StickyNote,
  ShieldCheck, Award, Clock, CreditCard, Boxes, Snowflake, HandCoins, BadgeCheck,
  ChevronDown, Search, Plus, Minus, CalendarIcon, Sparkles, ArrowRight,
  Leaf, Drumstick, ShoppingBasket, Cookie, UtensilsCrossed, Info, Store, Check,
  Download, Trash2, FileText, ListPlus
} from "lucide-react";
...
const BulkOrders = () => {
    ...
    setForm(prev => {
      ...
      if (field === "product") {
        ...
        if (prod) {
          ...
          next.unit = units[0] || "";
          next.brand = prod.brandVariants?.[0]?.brand || "";
        }
      }
      return next;
    });
  ...
  const handleAddToList = () => {
    ...
    setForm(prev => ({ ...prev, product: "", brand: "", quantity: 1, unit: "" }));
    // Scroll to list
  ...
  const handleDownloadCSV = () => {
    ...
    const rows = orderItems.map((item, i) => [
      i + 1, item.productName, item.brand, item.quantity, item.unit,
      `₹${item.price}`, `${item.discount}%`, `₹${item.total.toFixed(0)}`
    ]);
  ...
  return (
    <Layout>
      ...
      <section className="relative overflow-hidden hero-gradient py-20 md:py-28">
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "radial-gradient(circle at 20% 50%, hsl(var(--primary-foreground) / 0.15) 0%, transparent 50%), radial-gradient(circle at 80% 50%, hsl(var(--secondary) / 0.2) 0%, transparent 50%)" }} />
        <motion.div className="container relative z-10" initial="hidden" animate="visible" variants={stagger}>
          <motion.div variants={fadeUp} className="inline-flex items-center gap-2 rounded-full border border-primary-foreground/20 bg-primary-foreground/10 px-4 py-1.5 text-xs font-semibold text-primary-foreground backdrop-blur-sm mb-6">
          ...
          <motion.div variants={fadeUp} className="flex flex-wrap gap-3">
            ...
            <a href="https://wa.me/919999999999?text=Hi%20Mr.Pitani,%20I%20want%20to%20discuss%20bulk%20supply" target="_blank" rel="noopener noreferrer"
       ...
       <section id="order-form" className="relative py-16 overflow-hidden">
        ...
        <div className="container">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-10">
           ...
           <motion.form onSubmit={handleSubmit} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}
            ...
            <motion.div variants={fadeUp} className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea value={form.address} onChange={e => handleChange("address", e.target.value)} placeholder="Full delivery address..." rows={2} className={`${inputClass("address")} resize-none`} />
                </div>
              </div>
            </motion.div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {productCategories.map(({ label, value, icon: CatIcon }) => (
                  <button key={value} type="button"
                    onClick={() => { handleChange("category", value); handleChange("product", ""); }}
                          ...
                          filteredProducts.map(p => {
                            ...
                            return (
                              <button key={p.id} type="button"
                                onClick={() => { handleChange("product", p.name); setDropdownOpen(false); setSearch(""); }}
                                ...
                                <span className="flex items-center gap-2">
                                  {p.type === "veg" ? (
                                    <span className="flex h-4 w-4 items-center justify-center rounded-sm border border-secondary text-secondary"><Leaf className="h-2.5 w-2.5" /></span>
                                  ) : (
                                    <span className="flex h-4 w-4 items-center justify-center rounded-sm border border-accent text-accent"><Drumstick className="h-2.5 w-2.5" /></span>
                                  )}
                                  {p.name}
                                </span>
                                <span className="flex items-center gap-2">
                                  {p.brandVariants && p.brandVariants.length > 1 && (
                                    <span className="text-[10px] text-primary font-medium px-1.5 py-0.5 rounded bg-primary/10">{p.brandVariants.length} brands</span>
                                  )}
                ...
                {hasBrandVariants && selectedProduct && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mb-4 overflow-hidden">
                    ...
                    <div className={`grid gap-2 ${
                      selectedProduct.brandVariants!.length <= 3 ? "grid-cols-3"
                        : selectedProduct.brandVariants!.length <= 4 ? "grid-cols-2 sm:grid-cols-4"
                        : "grid-cols-2 sm:grid-cols-3"
                    }`}>
                      {selectedProduct.brandVariants!.map((v) => {
                        const isSelected = form.brand === v.brand;
                        return (
                          <button key={v.brand} type="button" onClick={() => handleChange("brand", v.brand)}
                            className={`relative rounded-xl border p-3 text-left transition-all duration-200 ${
                              isSelected ? "border-primary bg-primary/8 shadow-sm ring-1 ring-primary/20" : "border-border bg-background hover:border-primary/30 hover:shadow-sm"
                            }`}>
                            {isSelected && (
                              <motion.div layoutId="bulk-brand-check" className="absolute -top-1.5 -right-1.5 h-5 w-5 rounded-full hero-gradient flex items-center justify-center shadow-sm">
                                <Check className="h-3 w-3 text-primary-foreground" />
                              </motion.div>
                            )}
                            <span className={`text-xs font-bold truncate block ${isSelected ? "text-primary" : "text-foreground"}`}>{v.brand}</span>
                            <span className={`text-sm font-extrabold ${isSelected ? "text-primary" : "text-foreground"}`}>₹{v.price}</span>
                          </button>
                        );
                      })}
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div>
                  ...
                  <div className="flex items-center rounded-xl border border-input bg-background overflow-hidden">
                    <button type="button" onClick={() => handleChange("quantity", Math.max(1, form.quantity - 1))}
                    ...
                    <button type="button" onClick={() => handleChange("quantity", form.quantity + 1)}
                      className="px-3 py-3 text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">
                ...
                <div>
                  ...
                  <div className="flex rounded-xl border border-input bg-background overflow-hidden">
                    {(availableUnits.length > 0 ? availableUnits : ["Kg"]).map(u => (
                      <button key={u} type="button" onClick={() => handleChange("unit", u)}
                ...
                <div className="flex flex-col justify-end">
                  <button type="button" onClick={handleAddToList} disabled={!selectedProduct}
              ...
              <AnimatePresence>
                {orderItems.length > 0 && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                    variants={fadeUp} className="mb-8">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
                        <FileText className="h-5 w-5 text-primary" /> Order List
                        <span className="text-xs font-medium text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{orderItems.length} items</span>
                      </h3>
                          ...
                          <tbody>
                            {orderItems.map((item, i) => (
                              <motion.tr key={item.id}
                                ...
                                <td className="px-4 py-3 text-center font-medium text-foreground">{item.quantity} {item.unit}</td>
            ...
            <motion.div variants={fadeUp} className="mb-8">
              ...
              <textarea value={form.notes} onChange={e => handleChange("notes", e.target.value)}
                placeholder="Any special requirements..." rows={3} className={`${inputClass("notes")} resize-none`} />
            </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
              ...
              <div className="rounded-xl bg-muted/50 p-4 mb-6 space-y-1.5 text-sm max-h-40 overflow-y-auto">
                {orderItems.map((item, i) => (
                  <div key={item.id} className="flex justify-between">
                    <span className="text-muted-foreground">{i + 1}. {item.productName} ({item.brand})</span>
                    <span className="font-semibold text-foreground">₹{item.total.toFixed(0)}</span>
                  </div>
                ))}
              ...
              <div className="space-y-2">
                ...
                <a href={`mailto:orders@mrpitani.com?subject=${emailSubject}&body=${emailBody}`}
